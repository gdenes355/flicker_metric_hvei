...
%
    ...
    if levels > default_levels
        warning('parameter "levels" to large!');
        res={};
        return;
    end
...
function res = gaussian_2D(X,K,jump)
    % TODO: This can be made more efficient by skipping every second sample
    res2 = imfilter( X, K, 'symmetric' );
    res = imfilter( res2, K', 'symmetric' );
